...
public class StandardOreConfig {
    ...
    public StandardOreConfig(int veinSize, int spawnRate, int spawnMin, int spawnMax, boolean isRare, ForgeConfigSpec.Builder builder, String id) {
        builder.push(id);
            GENERATE = builder.comment("Whether to generate " + id + " or not").define("generate_" + id,true);
            VEIN_SIZE = builder.comment("Size of the veins for " + id).defineInRange(id + "_vein_size",veinSize,1,64);
